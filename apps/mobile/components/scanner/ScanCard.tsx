...
type Props = {
  ...
  blurb: string;
  onPress: () => void;
  live?: boolean; // Body scan is live — show a LIVE badge + open the real scanner
};
...
export function ScanCard({ mode, title, blurb, onPress, live }: Props) {
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [s.card, live && s.cardLive, pressed && { opacity: 0.7 }]}
      accessibilityRole="button"
      accessibilityLabel={live ? `${title} scan — open` : `${title} scan — rolling out soon`}
    >
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        <View style={s.titleRow}>
          ...
          <View style={live ? s.liveBadge : s.soonBadge}>
            <Text style={live ? s.liveText : s.soonText}>{live ? "LIVE" : "SOON"}</Text>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  cardLive: { backgroundColor: "rgba(52,211,153,0.06)", borderColor: "rgba(52,211,153,0.28)" },
  iconWrap: {
    width: 46,
    height: 46,
    borderRadius: 14,
  ...
  liveText: { color: "#34D399", fontSize: 8, fontWeight: "800", letterSpacing: 1.6 },
