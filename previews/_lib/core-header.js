  ...
  function render() {
    ...
    document.querySelectorAll('.core-header-actions').forEach(host => {
      ...
      const parts = show.map(key => {
        ...
        if (key === 'bell') {
          var unread=0;try{var n=JSON.parse(localStorage.getItem('coreNotifications')||'[]');unread=n.filter(function(x){return !x.read}).length}catch(e){}
          return (
          '<div class="ch-icon-btn" onclick="window.location.href=\'71-notifications.html\'" aria-label="Notifications">' +
            ...
            (unread>0?'<span class="ch-badge" style="position:absolute;top:-2px;right:-2px;min-width:16px;height:16px;border-radius:8px;background:#FF453A;color:#fff;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;padding:0 4px">'+unread+'</span>':'') +
          '</div>'
        );}
